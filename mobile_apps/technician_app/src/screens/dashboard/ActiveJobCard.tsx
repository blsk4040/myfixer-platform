...
// Note: Ensure this relative import correctly targets your local dashboard types file
import { ActiveJob } from '../../screens/dashboard/types/dashboard';
  ...
  if (!job) {
    return (
      <View style={[styles.container, styles.emptyState]}>
        <Text style={styles.emptyText}>No Active Job Assigned</Text>
      </View>
    );
  }

  const handleOpenNavigation = () => {
    // FIX: Read 'fullAddress' to perfectly match your type definition
    ...
    Linking.canOpenURL(mapsUrl)
      .then((supported) => {
        if (supported) {
          ...
          Alert.alert('Error', 'Unable to launch native mapping applications.');
        }
      });
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.badgeRow}>
        <Text style={styles.badgeText}>⚡ ACTIVE ASSIGNMENT</Text>
      ...
      {/* FIX: Read 'fullAddress' instead of 'address' */}
      ...
      <TouchableOpacity style={styles.actionButton} onPress={handleOpenNavigation}>
        <Text style={styles.actionButtonText}>Open Navigation Map</Text>
      </TouchableOpacity>
    </View>
  );
}

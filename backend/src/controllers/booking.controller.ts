...
import matchingService, { normalizeDispatchServiceKey, RankedTechnicianCandidate } from '../services/matching.service';
...
const notifyTechniciansForBooking = async (
...
const scheduleStandbyProviderRetries = (
  bookingId: string,
  io: SocketIOServer | undefined,
  retryUntil = new Date(Date.now() + STANDBY_RETRY_WINDOW_MS)
): void => {
  ...
  const timer = setTimeout(async () => {
    try {
      ...
      if (stopAt.getTime() <= now.getTime()) return;
...
export const createBooking = async (
  ...
  try {
    const promotionResolution = await resolvePromotionsForPricing({
      promoCode,
      customerId,
    ...
    if (error instanceof PromotionCampaignError || promoCode) {
...
export const confirmArrival = async (
  ...
  try {
    ...
    await createCustomerBookingNotification(
      ...
      `Your ${providerRole} has arrived for ${booking.applianceType}. Padi keeps job communication, approvals and payment steps together.`
    );

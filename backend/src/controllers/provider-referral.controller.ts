...
import { getOrCreateProviderReferralCode, getProviderReferralSummary } from '../services/provider-referral.service';
...
export const getMyProviderReferralProgram = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const technician = await Technician.findOne({ userId }).populate('userId', 'name').exec();
    if (!technician) {
